...
const video_pattern = /^((?:https?:)?\/\/)?(?:(?:www|m|music)\.)?((?:youtube\.com|youtu.be))(\/(?:[\w\-]+\?v=|shorts\/|embed\/|v\/)?)([\w\-]+)(\S+)?$/;
...
async function video_basic_info(url, options = {}) {
    ...
    const musicInfo = initial_response.engagementPanels
        .find((item) => item?.engagementPanelSectionListRenderer?.panelIdentifier == 'engagement-panel-structured-description')
        ?.engagementPanelSectionListRenderer.content.structuredDescriptionContentRenderer.items.find((el) => el.videoDescriptionMusicSectionRenderer)?.videoDescriptionMusicSectionRenderer.carouselLockups;
    const music = [];
    if (musicInfo) {
        musicInfo.forEach((x) => {
            ...
            const song = row.videoLockup?.compactVideoRenderer.title.simpleText ??
                row.videoLockup?.compactVideoRenderer.title.runs?.find((x) => x.text)?.text;
            const metadata = row.infoRows?.map((info) => [
            ...
            const id = row.videoLockup?.compactVideoRenderer.navigationEndpoint?.watchEndpoint.videoId ??
                row.infoRows
                    ?.find((x) => x.infoRowRenderer.title.simpleText.toLowerCase() == 'song')
                    ?.infoRowRenderer.defaultMetadata.runs?.find((x) => x.navigationEndpoint)?.navigationEndpoint
    ...
    const likeRenderer = initial_response.contents.twoColumnWatchNextResults.results.results.contents
        ...
        ?.videoPrimaryInfoRenderer.videoActions.menuRenderer.topLevelButtons?.find((button) => button.toggleButtonRenderer?.defaultIcon.iconType === 'LIKE' ||
    ...
    const video_details = new Video_1.YouTubeVideo({
        ...
        likes: parseInt(likeRenderer?.toggleButtonRenderer?.defaultText.accessibility?.accessibilityData.label.replace(/\D+/g, '') ??
            likeRenderer?.segmentedLikeDislikeButtonRenderer?.likeButton.toggleButtonRenderer?.defaultText.accessibility?.accessibilityData.label.replace(/\D+/g, '') ??
            0),
    ...
    if (!upcoming) {
        ...
        if ((0, stream_1.parseAudioFormats)(format).length === 0 && !options.htmldata) {
